...
struct Tree {
    ...
    Tree() {}
    Tree(int _n) : n(_n), adj(n + 1), parent(n + 1, -1), tin(n + 1), tout(n + 1), timer(0) {}
...
bool zapytaj(int x) {
    ...
    if (x < 1 || x > __judge::n) __judge::fail("Query value outside of range [1, n]");
    if (++__judge::query > __judge::MAX_QUERIES) __judge::fail("Too many queries");
    bool result = __judge::tree.is_ancestor(x, __judge::capek_position);
    if (!result) {
        if (__judge::capek_position != 1 && __judge::tree.parent[__judge::capek_position] != -1) {
...
int main() {
    ...
    for (int test = 0; test < t; test++) {
        ...
        if (x != __judge::capek_position) __judge::fail("Wrong Capek position guessed");
